...
@REM -- Windows NT with Amd64, ARM64 or MSVC VER >= 1400 knows about /Bxxx
if "%PROCESSOR_ARCHITECTURE%"=="AMD64" goto isAmd64
if "%PROCESSOR_ARCHITECTURE%"=="ARM64" goto isArm64
if not "%PROCESSOR_ARCHITEW6432%"=="" (
    if "%PROCESSOR_ARCHITEW6432%"=="AMD64" goto isAmd64
    if "%PROCESSOR_ARCHITEW6432%"=="ARM64" goto isArm64
)
goto checkMSVC
...
if not "%VisualStudioVersion%" == "" (
    if not "%VisualStudioVersion%" LSS "14" (
        set MAVEN_CMD_LINE_ARGS=%*
        goto endInit
    )
)

@REM Reaching here means no compatible version of Visual Studio was found
set MAVEN_CMD_LINE_ARGS=%*
goto endInit
