...
const CurdContainer = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const response = await fetch(`https://10.200.1.34/api/users`);
    ...
    try {
      ...
      if (editIndex !== null) {
        ...
        response = await fetch(
          `https://10.200.1.34/api/users/${products[editIndex].id}`,
          {
            method: "PUT",
            headers: {
        ...
        response = await fetch(`https://10.200.1.34/api/users`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  const handleDelete = async (id: number) => {
    try {
      setError(null);
      const response = await fetch(`https://10.200.1.34/api/users/${id}`, {
        method: "DELETE",
      });
